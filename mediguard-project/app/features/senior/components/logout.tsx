...
export default function LogoutScreen() {
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      if (ok) {
        Toast.show({ type: 'success', text1: '자격 확인 완료', position: 'bottom' });
        askFinalConfirm();
      } else {
      ...
      Toast.show({ type: 'error', text1: '검증 중 오류가 발생했습니다', position: 'bottom' });
      setPhase('input');
    } finally {
      setBusy(false);
    }
          ...
          <View style={styles.row}>
            ...
            >
              {phase === 'verifying' ? (
                ...
                <Text style={styles.buttonText}>자격 확인</Text>
              )}
            </TouchableOpacity>
          </View>
        </>
